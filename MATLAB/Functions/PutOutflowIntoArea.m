...
%   Detailed explanation goes here
    ...
    if OutFlow(4,2) > 0 && OutFlow(3,2) > 0 && RowPosition + 1 <= Rows
    ...
    if OutFlow(5,2) > 0;
                WaterContentMap(RowPosition, ColumnPosition) =...
                   WaterContentMap(RowPosition, ColumnPosition) + OutFlow(5,2);
        AreaMapStructure(RowPosition, ColumnPosition).OutFlow(5,2) = 0;
    end
    
    if sum(AreaMapStructure(RowPosition, ColumnPosition).OutFlow(:,2)) < -0.0000001 ||...
            sum(AreaMapStructure(RowPosition, ColumnPosition).OutFlow(:,2)) > 0.0000001
